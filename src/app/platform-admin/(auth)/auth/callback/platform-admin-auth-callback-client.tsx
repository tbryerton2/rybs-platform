...
function createBrowserPlatformAdminAuthClient() {
  return createClient(
...
export function PlatformAdminAuthCallbackClient() {
  ...
  useEffect(() => {
    ...
    async function completePlatformAdminSignIn() {
      ...
      if (errorCode) {
        setState({
          status: "error",
          message: errorDescription || "Your platform admin sign-in link could not be verified.",
        });
        return;
      }
      ...
      if (code && !accessToken) {
        try {
          const authClient = createBrowserPlatformAdminAuthClient();
      ...
      if (!payload) {
        setState({
          status: "error",
          message:
            "This platform admin sign-in link is missing session data. Request a new link and make sure the full URL opens in this browser.",
        });
        return;
      }
      ...
      try {
        response = await fetch("/platform-admin/auth/session", {
          method: "POST",
          ...
          setState({
            ...
            message:
              error instanceof DOMException && error.name === "AbortError"
                ? "Platform admin sign-in took too long to finish. Request a new link and try again."
                : "We could not reach the platform admin session endpoint. Check your connection and request a new link.",
          });
        }
        return;
      } finally {
    ...
    void completePlatformAdminSignIn();
      ...
      {state.status === "processing" ? (
        <>
          <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-900">
            Finishing platform admin sign-in
          </h1>
          <p className="mt-3 text-sm leading-6 text-slate-500">
            We are validating your secure link and setting up your platform session.
          </p>
        </>
      ) : (
        <>
          ...
          <div className="mt-6">
            <Link href="/platform-admin/login" className="admin-btn admin-btn-primary">
