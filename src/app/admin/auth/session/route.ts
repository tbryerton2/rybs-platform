...
function logAdminAuthSessionError(event: string, details: Record<string, unknown>) {
  console.error("[admin-auth-session]", {
    event,
    ...details,
  });
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!body) {
      logAdminAuthSessionError("missing_payload", {});
      return badRequest("Admin callback payload was missing.");
    }
    ...
    if (body.tokenHash && body.type) {
      ...
      if (error || !data.session || !data.user) {
        logAdminAuthSessionError("verify_otp_failed", {
          ...payloadShape,
          message: error?.message ?? "No session or user returned.",
          status: error?.status ?? null,
          code: error?.code ?? null,
        });
        return badRequest("This admin sign-in link is invalid or expired.", 401);
      }
      ...
      if (error || !data.session || !data.user) {
        logAdminAuthSessionError("code_exchange_failed", {
          ...payloadShape,
          message: error?.message ?? "No session or user returned.",
          status: error?.status ?? null,
          code: error?.code ?? null,
        });
        return badRequest("This admin sign-in code is invalid or expired. Request a new link and try again.", 401);
      }
      ...
      if (error || !data.user) {
        logAdminAuthSessionError("access_token_validation_failed", {
          ...payloadShape,
    ...
    logAdminAuthSessionError("unexpected_error", {
      message: error instanceof Error ? error.message : "Unknown error",
      stack: error instanceof Error ? error.stack : null,
    });
    return badRequest("Admin session setup failed.", 500);
  }
}
