...
with params as (
  select
    ...
    case
      when p_bucket_granularity in ('daily', 'weekly', 'monthly') then p_bucket_granularity
      else 'daily'
  ...
  select
    case
      when p.current_start_at is not null then
        date_trunc(p.bucket_granularity, p.current_start_at at time zone p.time_zone)
      else
        coalesce(
          (
            select date_trunc(p.bucket_granularity, min(b.created_at) at time zone p.time_zone)
            from current_bookings b
          ),
          date_trunc(p.bucket_granularity, (p.current_end_at - interval '1 day') at time zone p.time_zone)
        )
    end as start_bucket,
    date_trunc(p.bucket_granularity, (p.current_end_at - interval '1 microsecond') at time zone p.time_zone) as end_bucket,
    case
      when p.bucket_granularity = 'monthly' then interval '1 month'
      when p.bucket_granularity = 'weekly' then interval '1 week'
      else interval '1 day'
...
trend_aggregate as (
  select
    date_trunc(tb.bucket_granularity, cb.created_at at time zone tb.time_zone)::timestamp as bucket_start,
