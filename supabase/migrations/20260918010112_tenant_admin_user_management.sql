alter table public.business_admin_memberships
  drop constraint if exists business_admin_memberships_role_check;

alter table public.business_admin_memberships
  add constraint business_admin_memberships_role_check
  check (role in ('owner', 'admin'));

comment on column public.business_admin_memberships.role is
  'Business admin access role. Phase 1 supports owner and admin only.';

create or replace function public.business_admin_grant_membership(
...
begin
  ...
  values (p_business_id, p_target_auth_user_id, p_role, 'active')
...
revoke all on function public.business_admin_grant_membership(uuid, uuid, uuid, text) from public;
revoke all on function public.business_admin_grant_membership(uuid, uuid, uuid, text) from anon;
revoke all on function public.business_admin_grant_membership(uuid, uuid, uuid, text) from authenticated;
grant execute on function public.business_admin_grant_membership(uuid, uuid, uuid, text) to service_role;
...
comment on function public.business_admin_grant_membership(uuid, uuid, uuid, text) is
  'Tenant owner-only service-role RPC for granting or reactivating business admin membership.';
